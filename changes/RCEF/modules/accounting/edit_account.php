...
$parentAccounts = $pdo->query("SELECT * FROM chart_of_accounts WHERE is_active = TRUE ORDER BY account_code")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("UPDATE chart_of_accounts
                          SET account_code = ?, account_name = ?, account_type = ?, parent_account = ?
                          WHERE id = ?");
...
<div class="container mt-4">
    ...
    <select name="parent_account" class="form-control">
        ...
        <?php foreach ($parentAccounts as $parent): ?>
            <option value="<?= $parent['id'] ?>" <?= $account['parent_account'] == $parent['id'] ? 'selected' : '' ?>>
