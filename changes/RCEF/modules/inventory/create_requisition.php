...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        redirect($base_url."/inventory/view_requisition.php?id=$requisitionId");
    } catch (Exception $e) {
...
$(document).ready(function() {
    ...
    function addItemRow(item = null) {
        ...
        const itemName = item ? `${item.item_code} - ${item.name}` : '';
                    ...
                    <select class="form-select item-select" name="items[${rowId}][item_id]" required>
                        ...
                        ${inventoryItems.map(i => `
                            <option value="${i.id}"
                                ...
                                ${i.item_code} - ${i.name} (${i.quantity_on_hand} ${i.unit_of_measure} available)
                            </option>
                        `).join('')}
                    </select>
                </td>
